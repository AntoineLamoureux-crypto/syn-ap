...
function SignUp() {
    ...
    try {
      if (userType === 'Employee') {
        const { data: res } = await axios.post(
          'http://localhost:9090/signUp/employee',
          employee
        );
        if (res.data === null) {
        ...
        const { data: res } = await axios.post(
          'http://localhost:9090/signUp/deliverer',
          deliverer
        );
        if (res.data === null) {
  ...
  return (
    <Container boxSize={'container.xl'} textAlign={'center'}>
      <Box display={'inline-flex'} my={'4'} mx={'auto'}>
        <Button onClick={() => setUserType('Deliverer')} mx="5" size={'lg'}>
          Deliverer
        </Button>
        <Button onClick={() => setUserType('Employee')} mx="5" size={'lg'}>
          Employee
        </Button>
      </Box>
      <Stack spacing="6">
        ...
        <FormControl id="username">
          ...
          <Input
            ...
            required
            value={
              userType === 'Deliverer' ? deliverer.username : employee.username
            }
            onChange={
              userType === 'Deliverer'
        ...
        <FormControl id="password">
          ...
          <Input
            ...
            required
            value={
              userType === 'Deliverer' ? deliverer.password : employee.password
            }
            onChange={
              userType === 'Deliverer'
                ? handleChangeDeliverer
        ...
        <FormControl id="lastName">
          ...
          <Input
            ...
            required
            value={
              userType === 'Deliverer' ? deliverer.lastName : employee.lastName
            }
            onChange={
              userType === 'Deliverer'
                ? handleChangeDeliverer
        ...
        <Button
          ...
          fontSize="md"
        >
          Sign up as {userType === 'Employee' ? 'an ' : 'a '}
          {userType}
        </Button>
        <Box>
